...
public final class RoadmapRuntime {
    ...
    public final class ConstructionRover extends Unit {
        ...
        ConstructionRover(Tile p){super(p,UnitType.ENGINEER,new Color(225,155,70), RoadmapRuntime.this.map);health=180;speed=.10f;}
    ...
    private void updateVasya(){
        if(vasya==null)return;if(targetWorkshop!=null&&inside==null){if(!vasya.isAlive()&&boarded){if(nearWorkshop(vasyaRover==null?null:vasyaRover.getPosition(),targetWorkshop))disembark();else vasyaRover.setTarget(freeNear(targetWorkshop.getPosition(),2));return;}if(nearWorkshop(vasya.getPosition(),targetWorkshop))enter(targetWorkshop);else moveVasyaToward(targetWorkshop.getPosition());return;}
    ...
    private boolean nearWorkshop(Tile tile,Workshop workshop){if(tile==null||workshop==null||workshop.getPosition()==null)return false;int minX=workshop.getPosition().getX(),minY=workshop.getPosition().getY();int maxX=minX+workshop.getFootprintWidth()-1,maxY=minY+workshop.getFootprintHeight()-1;int dx=tile.getX()<minX?minX-tile.getX():tile.getX()>maxX?tile.getX()-maxX:0;int dy=tile.getY()<minY?minY-tile.getY():tile.getY()>maxY?tile.getY()-maxY:0;return dx+dy<=1;}
